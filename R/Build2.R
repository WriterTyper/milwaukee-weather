...
ghcn <- read_csv("data/GHCN_IN020040900.csv") %>%
  filter(year < 2022)
...
max.graph <- daily.summary.stats %>%
  ...
  scale_x_continuous(expand = expansion(0),
                     ...
                     name = NULL) +
  labs(title = "Daily High Temperature at Madras Airport",
...
ggsave("graphs/DailyHighTemp_IN020040900_2021.png", plot = max.graph2,
       width = 8, height = 4)
